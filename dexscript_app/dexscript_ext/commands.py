...
class DexCommand:
    ...
    def attribute_error(self, model, attribute):
        ...
        # Check if attribute exists in the model's fields
        fields = [f.name for f in model.value._meta.get_fields() if hasattr(f, 'name')]
...
class Global(DexCommand):
    ...
    async def update(self, ctx, model, identifier, attribute, value=None):
        ...
        # Detect image fields: CharField with max_length=200
        image_fields = Utils.fetch_fields(
            model.value,
            lambda name, field: (
                field.__class__.__name__ == "CharField" and getattr(field, "max_length", 0) == 200
            ),
        )
        ...
        if attribute.type == Types.MODEL:
            # For FK fields, set the _id column directly
    ...
    async def view(self, ctx, model, identifier, attribute=None):
        ...
        if attribute is None:
            ...
            for key, val in vars(returned_model).items():
                if key.startswith("_"):
                    continue

                fields["content"] += f"{key}: {val}\n"

                if isinstance(val, str) and Utils.is_image(val):
                    fields.setdefault("files", []).append(discord.File(Utils.image_path(val)))
        ...
        if attribute.type == Types.MODEL:
            # For FK fields, fetch the related object's string representation
            related_obj = getattr(returned_model, attribute_name, None)
            if related_obj is not None:
                new_attribute = str(related_obj)
    ...
    async def attributes(self, ctx, model, filter=None):
        ...
        def filter_function(name, field):
            field_class = field.__class__.__name__ if not isinstance(field, str) else field

            if field_class == "BackwardFKRelation":
                return False
            ...
            match filter.value.lower():
                ...
                case "valid":
                    return not getattr(field, "null", False)
...
class Filter(DexCommand):
    ...
    async def update(self, ctx, model, attribute, old_value, new_value, tortoise_operator=None):
        ...
        meets the condition defined by the optional `OPERATOR` argument.
        ...
        if attribute.type == Types.MODEL:
            value_old = (await Utils.get_model(attribute, value_old)).pk
            value_new = (await Utils.get_model(attribute, value_new)).pk
            casing_name += "_id"

        await model.value.objects.filter(**{casing_name: value_old}).aupdate(
    ...
    async def delete(self, ctx, model, attribute, value, tortoise_operator=None):
        ...
        defined by the optional `OPERATOR` argument.
        ...
        if attribute.type == Types.MODEL:
            new_value = (await Utils.get_model(attribute, new_value)).pk
            casing_name += "_id"
    ...
    async def view(self, ctx, model, attribute, value, tortoise_operator=None):
        ...
        instances = [
            x
            async for x in model.value.objects.filter(**{casing_name: new_value}).values_list(
                str_field, flat=True
            )
        ]
...
class Eval(DexCommand):
    ...
    async def save(self, ctx, name):
        ...
        if len(name.name) > NAME_LIMIT:
            raise Exception(
                f"`{name}` exceeds the {NAME_LIMIT}-character limit ({len(name.name)} > {NAME_LIMIT})"
            )
...
class Template(DexCommand):
    """
    Template commands used to assist with DexComX commands.
    """
