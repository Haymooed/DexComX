...
class Utils:
    ...
    def image_path(path: str) -> str:
        ...
        """
        # Strip leading /media/ or /static/uploads/ prefixes
        full_path = path.replace("/static/uploads/", "").replace("/media/", "")
    ...
    async def message_list(ctx, messages: list[str]):
        """
        Creates an interactive message list that allows you to display a list of messages
        without surpassing the Discord message character limit.
        ...
        for message in messages:
            ...
            page_length += len(str(message))
            pages[-1].append(message)

        for index, page in enumerate(pages, start=1):
            await ctx.send(f"```\n{chr(10).join(str(m) for m in page)}\n```")
            ...
            await ctx.send(
                file=discord.File(
                    StringIO("\n".join(str(m) for m in messages)), filename="output.txt"
                )
            )
    ...
    def models(names=False, key: Callable | None = None):
        ...
        key: Callable | None
            The model instance or name will be passed through this callable per model.
        """
        model_list = MODELS
    ...
    async def create_model(model, identifier, fields_only=False):
        """
        Creates a model instance while providing default values for all required fields.

        Parameters
        ----------
        model: Model
            The Django model class you want to use.
        identifier: str
        ...
        identifier_fields = ["country", "catch_names", "name"]
        ignore_fields = ["id", "short_name", "created_at"]

        for field in model._meta.get_fields():
            field_class = field.__class__.__name__

            # Skip reverse relations
            if field_class in ("ManyToOneRel", "ManyToManyRel", "OneToOneRel", "ForeignObjectRel"):
                continue

            # Skip fields without a name (safety)
            if not hasattr(field, "name"):
                continue

            field_name = field.name

            # Skip auto fields, ignored fields, and nullable fields
            ...
            match field_class:
                case "ForeignKey":
                    ...
                    related_model = Utils.fetch_model(casing_field)
                    ...
                    # Use attname (e.g. regime_id) to set the FK directly
                    fields[field.attname] = instance.pk

                case "BigIntegerField" | "PositiveBigIntegerField":
                    fields[field_name] = 100**8

                case "IntegerField" | "SmallIntegerField" | "PositiveIntegerField":
                    fields[field_name] = 1
                ...
                case "ImageField" | "FileField":
                    # Skip image fields — they need to be set separately
                    continue
                ...
                case _:
                    # Skip unknown field types rather than setting bad defaults
                    continue
    ...
    async def get_model(model, identifier):
        ...
        ----------
        model: Value
            The model Value object (has .value = Django model class, .extra_data[0] = str field).
        identifier: str | Value
            The identifier of the model instance you are trying to return.
        """
        str_identifier = str(identifier)
        str_field = model.extra_data[0]

        correction_list = [
            x async for x in model.value.objects.values_list(str_field, flat=True)
        ]

        corrected = Utils.autocorrect(str_identifier, correction_list)

        try:
            returned_model = await model.value.objects.aget(**{str_field: corrected})
        except model.value.DoesNotExist:
            raise Exception(f"'{str_identifier}' does not exist.")
    ...
    def fetch_fields(model, field_filter: Callable | None = None) -> list[str]:
        """
        Returns a list of a model's field names.

        Parameters
        ----------
        model: Model
            The Django model class you want to fetch fields from.
        ...
        for field in model._meta.get_fields():
            ...
            # Skip reverse relations — they are not directly settable
            if field_class in ("ManyToOneRel", "ManyToManyRel", "OneToOneRel"):
                if field_filter is not None and not field_filter(field.name, "BackwardFKRelation"):
                    continue
                # By default, skip reverse relations
    ...
    def autocorrect(string: str, correction_list: list[str], error="does not exist."):
        ...
        and raises an error if there are no strings similar to the string provided.
